...
struct ProposalRefineStats {
  ...
  double feature_build_latency_ms = 0.0;
  double inference_latency_ms = 0.0;
  double delta_apply_latency_ms = 0.0;
...
class ProposalRefineModule {
 ...
 private:
  ...
  bool InitStudentModel(const centerpoint::ProposalRefineParam& config);
  bool DisableStudentModelAfterLoadFailure(const std::string& reason);
  ...
  StudentModel* student_model_ = nullptr;
};
